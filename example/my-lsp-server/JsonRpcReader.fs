...
module rec MyLspServer.JsonRpcReader
...
type private JsonRpcReaderHost = { Reader: BinaryReader }
...
let createJsonRpcReader () : Async<JsonValue> =
  let mutable current = newState ()

  async {
    let input = System.Console.OpenStandardInput()
    use reader = new BinaryReader(input)
    let host: JsonRpcReaderHost = { Reader = reader }

    let state = current
    let contentLengthOpt, state = readHeader host None state

    match contentLengthOpt with
    | None -> return! failwith "jsonRpcReader: Unexpected EOF"
